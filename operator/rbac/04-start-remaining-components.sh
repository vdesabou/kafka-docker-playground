...
if [[ $? -eq 1 ]]; then
  ...
  docker run --quiet --rm -v $PWD:/tmp ${CP_CONNECT_IMAGE}:${CONNECT_TAG} sed -i '/confluent.controlcenter.data.dir/a \          confluent.controlcenter.ui.autoupdate.enable=false' /tmp/confluent-operator/helm/confluent-operator/charts/controlcenter/templates/controlcenter-psc.yaml
  # docker run --quiet --rm -v $PWD:/tmp ${CP_CONNECT_IMAGE}:${CONNECT_TAG} sed -i '/confluent.controlcenter.data.dir/a \          confluent.controlcenter.service.healthcheck.interval.sec=600' /tmp/confluent-operator/helm/confluent-operator/charts/controlcenter/templates/controlcenter-psc.yaml
fi
...
if [[ $? -eq 1 ]]; then
  ...
  # use linux to run sed
  docker run --quiet --rm -v $PWD:/tmp ${CP_CONNECT_IMAGE}:${CONNECT_TAG} sed -i '/authentication.skip.paths/a \          access.control.allow.origin=*' /tmp/confluent-operator/helm/confluent-operator/charts/ksql/templates/ksql-psc.yaml
  docker run --quiet --rm -v $PWD:/tmp ${CP_CONNECT_IMAGE}:${CONNECT_TAG} sed -i '/authentication.skip.paths/a \          access.control.allow.methods=GET,POST,HEAD' /tmp/confluent-operator/helm/confluent-operator/charts/ksql/templates/ksql-psc.yaml
  docker run --quiet --rm -v $PWD:/tmp ${CP_CONNECT_IMAGE}:${CONNECT_TAG} sed -i '/authentication.skip.paths/a \          access.control.allow.headers=X-Requested-With,Content-Type,Accept,Origin,Authorization' /tmp/confluent-operator/helm/confluent-operator/charts/ksql/templates/ksql-psc.yaml

  docker run --quiet --rm -v $PWD:/tmp ${CP_CONNECT_IMAGE}:${CONNECT_TAG} sed -i '/log4j.rootLogger=INFO, stdout/a \          log4j.logger.processing=ERROR, kafka_appender' /tmp/confluent-operator/helm/confluent-operator/charts/ksql/templates/ksql-psc.yaml
  docker run --quiet --rm -v $PWD:/tmp ${CP_CONNECT_IMAGE}:${CONNECT_TAG} sed -i '/log4j.rootLogger=INFO, stdout/a \          log4j.appender.kafka_appender=org.apache.kafka.log4jappender.KafkaLog4jAppender' /tmp/confluent-operator/helm/confluent-operator/charts/ksql/templates/ksql-psc.yaml
  docker run --quiet --rm -v $PWD:/tmp ${CP_CONNECT_IMAGE}:${CONNECT_TAG} sed -i '/log4j.rootLogger=INFO, stdout/a \          log4j.appender.kafka_appender.layout=io.confluent.common.logging.log4j.StructuredJsonLayout' /tmp/confluent-operator/helm/confluent-operator/charts/ksql/templates/ksql-psc.yaml
  docker run --quiet --rm -v $PWD:/tmp ${CP_CONNECT_IMAGE}:${CONNECT_TAG} sed -i '/log4j.rootLogger=INFO, stdout/a \          log4j.appender.kafka_appender.BrokerList=kafka:9071' /tmp/confluent-operator/helm/confluent-operator/charts/ksql/templates/ksql-psc.yaml
  docker run --quiet --rm -v $PWD:/tmp ${CP_CONNECT_IMAGE}:${CONNECT_TAG} sed -i '/log4j.rootLogger=INFO, stdout/a \          log4j.appender.kafka_appender.Topic={{ .Release.Namespace }}.{{ $.Values.name }}_ksql_processing_log' /tmp/confluent-operator/helm/confluent-operator/charts/ksql/templates/ksql-psc.yaml
  docker run --quiet --rm -v $PWD:/tmp ${CP_CONNECT_IMAGE}:${CONNECT_TAG} sed -i '/log4j.rootLogger=INFO, stdout/a \          log4j.appender.kafka_appender.SyncSend=false' /tmp/confluent-operator/helm/confluent-operator/charts/ksql/templates/ksql-psc.yaml
  docker run --quiet --rm -v $PWD:/tmp ${CP_CONNECT_IMAGE}:${CONNECT_TAG} sed -i '/log4j.rootLogger=INFO, stdout/a \          log4j.appender.kafka_appender.SecurityProtocol=SASL_SSL' /tmp/confluent-operator/helm/confluent-operator/charts/ksql/templates/ksql-psc.yaml
  docker run --quiet --rm -v $PWD:/tmp ${CP_CONNECT_IMAGE}:${CONNECT_TAG} sed -i '/log4j.rootLogger=INFO, stdout/a \          log4j.appender.kafka_appender.SaslMechanism=PLAIN' /tmp/confluent-operator/helm/confluent-operator/charts/ksql/templates/ksql-psc.yaml
  docker run --quiet --rm -v $PWD:/tmp ${CP_CONNECT_IMAGE}:${CONNECT_TAG} sed -i '/log4j.rootLogger=INFO, stdout/a \          log4j.appender.kafka_appender.ClientJaasConf=org.apache.kafka.common.security.plain.PlainLoginModule required username="kafka" password="kafka-secret";' /tmp/confluent-operator/helm/confluent-operator/charts/ksql/templates/ksql-psc.yaml
  docker run --quiet --rm -v $PWD:/tmp ${CP_CONNECT_IMAGE}:${CONNECT_TAG} sed -i '/log4j.rootLogger=INFO, stdout/a \          log4j.appender.kafka_appender.SslKeystoreType=JKS' /tmp/confluent-operator/helm/confluent-operator/charts/ksql/templates/ksql-psc.yaml
  docker run --quiet --rm -v $PWD:/tmp ${CP_CONNECT_IMAGE}:${CONNECT_TAG} sed -i '/log4j.rootLogger=INFO, stdout/a \          log4j.appender.kafka_appender.SslTruststoreLocation=/tmp/truststore.jks' /tmp/confluent-operator/helm/confluent-operator/charts/ksql/templates/ksql-psc.yaml
  docker run --quiet --rm -v $PWD:/tmp ${CP_CONNECT_IMAGE}:${CONNECT_TAG} sed -i '/log4j.rootLogger=INFO, stdout/a \          log4j.appender.kafka_appender.SslTruststorePassword=mystorepassword' /tmp/confluent-operator/helm/confluent-operator/charts/ksql/templates/ksql-psc.yaml

  docker run --quiet --rm -v $PWD:/tmp ${CP_CONNECT_IMAGE}:${CONNECT_TAG} sed -i '/## ksql configuration/a \          ksql.logging.processing.rows.include=true' /tmp/confluent-operator/helm/confluent-operator/charts/ksql/templates/ksql-psc.yaml
  docker run --quiet --rm -v $PWD:/tmp ${CP_CONNECT_IMAGE}:${CONNECT_TAG} sed -i '/## ksql configuration/a \          ksql.logging.processing.stream.auto.create=true' /tmp/confluent-operator/helm/confluent-operator/charts/ksql/templates/ksql-psc.yaml
  docker run --quiet --rm -v $PWD:/tmp ${CP_CONNECT_IMAGE}:${CONNECT_TAG} sed -i '/## ksql configuration/a \          ksql.logging.processing.topic.auto.create=true' /tmp/confluent-operator/helm/confluent-operator/charts/ksql/templates/ksql-psc.yaml
  docker run --quiet --rm -v $PWD:/tmp ${CP_CONNECT_IMAGE}:${CONNECT_TAG} sed -i '/## ksql configuration/a \          ksql.logging.processing.topic.name={{ .Release.Namespace }}.{{ $.Values.name }}_ksql_processing_log' /tmp/confluent-operator/helm/confluent-operator/charts/ksql/templates/ksql-psc.yaml
fi
