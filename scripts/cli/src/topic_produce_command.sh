...
then
    ...
    head -n 1 "$output_value_file" | cut -c 1-${size_limit_to_show} | awk "{print \$0 \"...<truncated, only showing first $size_limit_to_show characters, out of $record_size>...\"}"
else
    if (( nb_generated_messages < 10 ))
    then
        log "✨ $nb_generated_messages records were generated$value_str"
        cat "$output_value_file"
    else
        log "✨ $nb_generated_messages records were generated$value_str (only showing first 10), $ELAPSED"
        head -n 10 "$output_value_file"
    fi
fi
...
then
    ...
    docker cp $tmp_dir/out_value.json connect:/tmp/message.json > /dev/null 2>&1
    env_list=""
...
then
    ...
    while read line
    do
        echo "${headers}|${line}" >> $tmp_dir/temp_value_file
    done < $output_value_file
            ...
            then
                ...
                then
                    ...
                    then
                        ...
                        head -n $nb_messages_to_send $output_value_file | docker exec -i $container kafka-console-producer --broker-list $bootstrap_server --topic $topic $security $producer_properties --property parse.headers=true --property headers.delimiter="|" --property headers.separator="," --property headers.key.separator=":"
                    else
                        if [[ -n "$verbose" ]]
                        then
                            set -x
                        fi
                        head -n $nb_messages_to_send $output_value_file | docker exec -i $container kafka-console-producer --broker-list $bootstrap_server --topic $topic $security $producer_properties
                    fi
                fi
            fi
        ;;
