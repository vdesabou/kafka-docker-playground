...
do
    ...
    if [ $ret -eq 0 ]
    then
        if echo "$json_config" | jq 'if .error then .error | has("code") else has("error_code") end' 2> /dev/null | grep -q true
        then
            if echo "$json_config" | jq '.error | has("code")' 2> /dev/null | grep -q true
            then
                code=$(echo "$json_config" | jq -r .error.code)
                message=$(echo "$json_config" | jq -r .error.message)
            else
                code=$(echo "$json_config" | jq -r .error_code)
