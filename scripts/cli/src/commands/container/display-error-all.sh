...
if [ -z "$containers" ]; then
    ...
    then
        ...
        log "####################################################"
        log "☸️ kubectl get events (latest 120)"
        kubectl -n confluent get events --sort-by=.metadata.creationTimestamp | tail -n 120
        ...
        do
           ...
           # restart_count=$(kubectl -n confluent get pod "$container" -o jsonpath='{range .status.containerStatuses[*]}{.restartCount}{" "}{end}' 2>/dev/null | tr ' ' '\n' | awk '$1>0{sum+=$1} END{print sum+0}')
            ...
            then
                ...
                log "####################################################"
                log "☸️ kubectl logs $container --all-containers --tail=200"
                kubectl -n confluent logs "$container" --all-containers --tail=200
                log "####################################################"
                log "☸️ kubectl logs $container --all-containers --previous --tail=200"
                kubectl -n confluent logs "$container" --all-containers --previous --tail=200
            fi
        done <<< "$containers"
