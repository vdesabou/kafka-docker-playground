...
gh_msg_file_final=$tmp_dir/gh_final.txt
...
do
  ...
  TEST_SUCCESS=()
  TEST_SKIPPED=()
  unset TEST_TOTAL_COUNT TEST_SKIPPED_COUNT TEST_FAILED_COUNT TEST_SUCCESS
  ...
  rm -f ${gh_msg_file_final}
  touch ${gh_msg_file_final}
  gh_issue_number=""
  if [ ! -d $test ]
  then
    # logwarn "####################################################"
  ...
  then
    ...
    then
      ...
      if [ -f "$gh_msg_file" ] && [ $(wc -c < "$gh_msg_file" | awk '{print $1}') -gt $MAX_SIZE ]
      then
          logwarn "✂️ log file $gh_msg_file exceeds $MAX_SIZE characters. Truncating..."
          # Keep only the first $MAX_SIZE bytes/characters and overwrite the file
    ...
    then
      ...
      then
        ci_cfk="[![CI_CFK ok](https://img.shields.io/badge/$nb_success/$nb_tests-cfk_ok!-green)]($html_url)"
      elif [ ${ci_cfk_nb_fail} -eq ${nb_image_versions} ]
      then
        ci_cfk="[![CI_CFK fail](https://img.shields.io/badge/$nb_success/$nb_tests-cfk_fail!-red)](https://github.com/vdesabou/kafka-docker-playground/issues/$gh_issue_number)"
      fi
    else
