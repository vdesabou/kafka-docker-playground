...
do
  ...
  do
    ...
    if [ -f "$ci_output_file" ] && [ $(wc -c < "$ci_output_file" | awk '{print $1}') -gt $MAX_SIZE ]
    then
        logwarn "✂️ log file $ci_output_file exceeds $MAX_SIZE characters. Truncating..."
        # Keep only the first $MAX_SIZE bytes/characters and overwrite the file
        head -c $MAX_SIZE "$ci_output_file" > "$ci_output_file.tmp" && mv "$ci_output_file.tmp" "$ci_output_file"
    fi
