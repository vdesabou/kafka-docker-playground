...
do
    ...
    then
        log "☸️ kubectl annotate connector $connector -n confluent platform.confluent.io/pause-connector=true --overwrite"
        kubectl annotate connector "$connector" -n confluent platform.confluent.io/pause-connector="true" --overwrite >/dev/null
    else
