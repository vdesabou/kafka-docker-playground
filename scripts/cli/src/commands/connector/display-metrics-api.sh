...
format_metric_value() {
    ...
    then
        ...
        local ts_human
        ts_human=$(date -r "$ts_seconds" "+%Y-%m-%d %H:%M:%S" 2>/dev/null || true)
        if [[ -n "$ts_human" ]]
        then
            echo "$printable_value (at $ts_human)"
            return
        fi
    fi
...
display_metrics_summary() {
    ...
    do
        ...
        then
            ...
            if [[ -n "$metric_context" ]]
            then
                echo " - ${display_name} (${metric_context}): $(format_metric_value "$metric_value" "$metric_ts")"
            else
                echo " - ${display_name}: $(format_metric_value "$metric_value" "$metric_ts")"
            fi
            ((displayed+=1))
        fi
