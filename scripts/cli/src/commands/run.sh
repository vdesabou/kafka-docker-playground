...
then
  ...
  then
    readonly MENU_ENABLE_KSQLDB="🚀 Enable ksqlDB" #0
    readonly MENU_ENABLE_C3="💠 Enable Control Center"
    readonly MENU_ENABLE_CONDUKTOR="🐺 Enable Conduktor Platform"
    readonly MENU_ENABLE_RP="🧲 Enable Rest Proxy" #3
    readonly MENU_ENABLE_GRAFANA="📊 Enable Grafana, Prometheus and Pyroscope"
    ...
    readonly MENU_ENVIRONMENT="🔐 The environment to start when running a connector example" #9
    readonly MENU_TAG="🎯 Confluent Platform (CP) version to use"
    readonly MENU_CONNECTOR_TAG="🔗 Connector version to use" #11
    readonly MENU_CONNECTOR_ZIP="🤐 Connector zip to use"
    readonly MENU_CONNECTOR_JAR="♨️ Connector jar to use"

    readonly MENU_CLUSTER_TYPE="🔋 The cluster type" #14
    ...
    then
      cluster_region=$(playground get-kafka-region-list)
