...
_playground_completions() {
  ...
  case "$compline" in
    ...
    'connector'*)
      while read -r; do COMPREPLY+=( "$REPLY" ); done < <( compgen -W "$(_playground_completions_filter "--help -h create-or-update delete log-level open-docs pause plugins restart resume search-jar show-config show-config-parameters show-lag snippets status stop unpause versions")" -- "$cur" )
      ;;
