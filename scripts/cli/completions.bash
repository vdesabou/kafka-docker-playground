...
_playground_completions() {
  ...
  case "$compline" in
    'bootstrap-reproduction-model'*'--producer-schema-value')
      while read -r; do COMPREPLY+=( "$REPLY" ); done < <( compgen -W "$(_playground_completions_filter "$(playground get-schema-with-fzf "$cur")")" -- "$cur" )
      ;;

    'bootstrap-reproduction-model'*'--producer-schema-key')
      while read -r; do COMPREPLY+=( "$REPLY" ); done < <( compgen -W "$(_playground_completions_filter "$(playground get-schema-with-fzf "$cur")")" -- "$cur" )
      ;;
    ...
    'get-zip-or-jar-with-fzf'*'--type')
      while read -r; do COMPREPLY+=( "$REPLY" ); done < <( compgen -W "$(_playground_completions_filter "zip jar")" -- "$cur" )
      ;;
    ...
    'bootstrap-reproduction-model'*)
      while read -r; do COMPREPLY+=( "$REPLY" ); done < <( compgen -W "$(_playground_completions_filter "--connector-jar --connector-tag --connector-zip --custom-smt --description --disable-control-center --disable-ksqldb --enable-conduktor --enable-jmx-grafana --enable-kcat --enable-multiple-brokers --enable-multiple-connect-workers --enable-sql-datagen --enable-sr-maven-plugin-app --file --help --nb-producers --pipeline --producer --producer-schema-key --producer-schema-value --tag -d -f -h -n -p")" -- "$cur" )
      ;;
    ...
    'get-examples-list-with-fzf'*)
      while read -r; do COMPREPLY+=( "$REPLY" ); done < <( compgen -W "$(_playground_completions_filter "--ccloud-only --help --sink-only --without-repro -h")" -- "$cur" )
      ;;
    ...
    'get-zip-or-jar-with-fzf'*)
      while read -r; do COMPREPLY+=( "$REPLY" ); done < <( compgen -W "$(_playground_completions_filter "--help --type -h")" -- "$cur" )
      ;;
    ...
    'run-ccloud'*)
      while read -r; do COMPREPLY+=( "$REPLY" ); done < <( compgen -W "$(_playground_completions_filter "--cluster-cloud --cluster-creds --cluster-environment --cluster-name --cluster-region --cluster-schema-registry-creds --connector-jar --connector-tag --connector-zip --disable-control-center --enable-conduktor --enable-kcat --file --help --open --tag -f -h -o")" -- "$cur" )
      ;;
    ...
    *)
      while read -r; do COMPREPLY+=( "$REPLY" ); done < <( compgen -W "$(_playground_completions_filter "--help --version -h -v bashly-reload bootstrap-reproduction-model connector container enable-remote-debugging get-all-schemas get-connector-list get-examples-list-with-fzf get-jmx-metrics get-properties get-schema-with-fzf get-topic-list get-zip-or-jar-with-fzf log-level open re-run run run-ccloud topic")" -- "$cur" )
      ;;
