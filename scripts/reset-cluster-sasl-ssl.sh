...
docker-compose -f docker-compose-sasl-ssl.yml down -v 
...
then
  ...
  while [[ ! $(docker-compose -f docker-compose-sasl-ssl.yml logs connect) =~ "Finished starting connectors and tasks" ]]; do
    sleep 10
    CUR_WAIT=$(( CUR_WAIT+10 ))
    if [[ "$CUR_WAIT" -gt "$MAX_WAIT" ]]; then
      echo -e "\nERROR: The logs in connect container do not show 'Finished starting connectors and tasks' after $MAX_WAIT seconds. Please troubleshoot with 'docker-compose ps' and 'docker-compose -f docker-compose-sasl-ssl.yml logs'.\n"
      exit 1
    fi
  done
...
then
  ...
  while [[ ! $(docker-compose -f docker-compose-sasl-ssl.yml logs control-center) =~ "Started NetworkTrafficServerConnector" ]]; do
    sleep 10
    CUR_WAIT=$(( CUR_WAIT+10 ))
    if [[ "$CUR_WAIT" -gt "$MAX_WAIT" ]]; then
      echo -e "\nERROR: The logs in control-center container do not show 'Started NetworkTrafficServerConnector' after $MAX_WAIT seconds. Please troubleshoot with 'docker-compose ps' and 'docker-compose -f docker-compose-sasl-ssl.yml logs'.\n"
      exit 1
    fi
  done
