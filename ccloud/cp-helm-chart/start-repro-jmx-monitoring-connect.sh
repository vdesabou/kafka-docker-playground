...
####
# Trick to use https://github.com/confluentinc/jmx-monitoring-stacks connect prometheus/grafana with cp-helm-charts
cp ${DIR}/repro-jmx-monitoring-connect/deployment.yaml ${DIR}/cp-helm-charts/charts/cp-kafka-connect/templates/deployment.yaml
cp ${DIR}/repro-jmx-monitoring-connect/jmx-configmap.yaml ${DIR}/cp-helm-charts/charts/cp-kafka-connect/templates/jmx-configmap.yaml
###
...
helm upgrade --install \
  ...
  --set imageTag="${TAG}" \
  --set heapOptions="-javaagent:/usr/share/jmx_prometheus_javaagent-0.12.0.jar=5556:/etc/jmx-kafka-connect/jmx-kafka-connect-prometheus.yml -Xms512M -Xmx512M" \
...
log "Then you can import dashboard in ${DIR}/repro-jmx-monitoring-connect/connect.json"