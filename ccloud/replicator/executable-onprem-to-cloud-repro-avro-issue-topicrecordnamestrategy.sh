...
then
     ...
     docker run -i --rm -e TAG=$TAG_BASE -e KAFKA_CLIENT_TAG=$KAFKA_CLIENT_TAG -v "${DIR}/producer-onprem":/usr/src/mymaven -v "$HOME/.m2":/root/.m2 -v "${DIR}/producer-onprem/target:/usr/src/mymaven/target" -w /usr/src/mymaven maven:3.6.1-jdk-11 mvn package
fi

if [ ! -f ${DIR}/producer-cloud/target/producer-1.0.0-jar-with-dependencies.jar ]
then
     log "Building jar for producer-cloud"
     docker run -i --rm -e TAG=$TAG_BASE -e KAFKA_CLIENT_TAG=$KAFKA_CLIENT_TAG -v "${DIR}/producer-cloud":/usr/src/mymaven -v "$HOME/.m2":/root/.m2 -v "${DIR}/producer-cloud/target:/usr/src/mymaven/target" -w /usr/src/mymaven maven:3.6.1-jdk-11 mvn package
fi
...
docker-compose -f ../../ccloud/environment/docker-compose.yml -f ${PWD}/docker-compose-executable-onprem-to-cloud-topicrecordnamestrategy.yml -f docker-compose-executable-onprem-to-cloud-repro-avro.yml up -d
