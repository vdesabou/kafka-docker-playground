...
until docker run \
    ...
    bash -c "cd LocalFunctionProj && npm install -g azure-functions-core-tools@$AZURE_FUNCTIONS_CORE_TOOLS_VERSION --unsafe-perm && npm install && func azure functionapp publish \"$AZURE_FUNCTIONS_NAME\" --access-token $TOKEN"
do
...
do
    output=$(docker run --platform linux/amd64 -v $PWD/LocalFunctionProj:/LocalFunctionProj mcr.microsoft.com/azure-functions/node:4-node22 bash -c "cd LocalFunctionProj && npm install -g azure-functions-core-tools@$AZURE_FUNCTIONS_CORE_TOOLS_VERSION --unsafe-perm && npm install && func azure functionapp list-functions \"$AZURE_FUNCTIONS_NAME\" --show-keys --access-token \"$TOKEN\"")
