...
docker run -v $PWD:/tmp vdesabou/kafka-docker-playground-connect:${CONNECT_TAG} openssl genrsa -out /tmp/ca-key.pem 2048
docker run -v $PWD:/tmp vdesabou/kafka-docker-playground-connect:${CONNECT_TAG} openssl req -new -key /tmp/ca-key.pem -x509 \
  -days 1000 \
  -out /tmp/ca.pem \
