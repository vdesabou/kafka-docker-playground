...
curl -s -X PUT \
      ...
      http://localhost:8083/connectors/datagen-ratings/config | jq .

wait_for_datagen_connector_to_inject_data "ratings" "1"

log "Creating Databricks Delta Lake Sink connector with one topic"
curl -X PUT \
     -H "Content-Type: application/json" \
     --data '{
...
log "Updating Databricks Delta Lake Sink connector with two topic"
curl -X PUT \
     -H "Content-Type: application/json" \
     --data '{
               "connector.class": "io.confluent.connect.databricks.deltalake.DatabricksDeltaLakeSinkConnector",
               "topics": "pageviews,ratings",
               "s3.region": "'"$DATABRICKS_AWS_BUCKET_REGION"'",
               "key.converter": "org.apache.kafka.connect.storage.StringConverter",
               "value.converter": "io.confluent.connect.avro.AvroConverter",
               "value.converter.schema.registry.url": "http://schema-registry:8081",
               ...
               "delta.lake.http.path": "'"$DATABRICKS_HTTP_PATH"'",
               "delta.lake.token": "'"$DATABRICKS_TOKEN"'",
               "delta.lake.topic2table.map": "pageviews:pageviews,ratings:ratings",
               "delta.lake.table.auto.create": "true",
               "staging.s3.access.key.id": "'"$DATABRICKS_AWS_STAGING_S3_ACCESS_KEY_ID"'",
               "staging.s3.secret.access.key": "'"$DATABRICKS_AWS_STAGING_S3_SECRET_ACCESS_KEY"'",
               "staging.bucket.name": "'"$DATABRICKS_AWS_BUCKET_NAME"'",
...
#                "transforms.InsertField.static.field" : "AddedField",
#                "transforms.InsertField.static.value" : "yes"
#             }' \
#       http://localhost:8083/connectors/datagen-ratings/config | jq .
