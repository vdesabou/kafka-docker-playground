...
# The easy work around would be to copy instead of moving the file and deleting the older file once the connector has been successfully updated.
log "Workaround: Copying JKS files and kafkajks.txt to new folder /tmp"
docker exec connect cp /etc/kafka/secrets/kafka.ftps-server.truststore.jks /tmp/
docker exec connect cp /etc/kafka/secrets/kafka.ftps-server.keystore.jks /tmp/
docker exec connect cp /etc/kafka/secrets/kafkajks.txt /tmp/
...
log "Now removing files from /etc/kafka/secrets"
docker exec connect rm /etc/kafka/secrets/kafka.ftps-server.truststore.jks
docker exec connect rm /etc/kafka/secrets/kafka.ftps-server.keystore.jks
docker exec connect rm /etc/kafka/secrets/kafkajks.txt

log "Update connector (changing task number)"
curl -X PUT \
     -H "Content-Type: application/json" \
     --data '{
               "tasks.max": "2",
