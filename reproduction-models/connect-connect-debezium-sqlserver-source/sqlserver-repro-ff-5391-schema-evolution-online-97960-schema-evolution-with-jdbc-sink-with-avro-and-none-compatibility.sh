...
# Note: if doing ALTER TABLE customers ADD phone_number VARCHAR(32) NOT NULL; we get:

# 07:55:58 ℹ️ alter table (following https://debezium.io/documentation/reference/connectors/sqlserver.html#online-schema-updates)
# Changed database context to 'testDB'.
# Msg 4901, Level 16, State 1, Server sqlserver, Line 2
# ALTER TABLE only allows columns to be added that can contain nulls, or have a DEFAULT definition specified, or the column being added is an identity or timestamp column, or alternatively if none of the previous conditions are satisfied the table must be empty to allow addition of this column. Column 'phone_number' cannot be added to non-empty table 'customers' because it does not satisfy these conditions.
