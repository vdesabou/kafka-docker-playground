...
log "Verify data was sent to broker using --property proxy.host=nginx-proxy -property proxy.port=8888"
timeout 60 docker exec connect kafka-avro-console-consumer -bootstrap-server broker:9092 --property proxy.host=nginx-proxy -property proxy.port=8888 --property schema.registry.url=http://schema-registry:8081 --topic customer_avro --from-beginning --max-messages 10