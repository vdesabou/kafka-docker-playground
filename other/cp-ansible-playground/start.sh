...
verify_installed "ssh-keygen"
verify_installed "ansible"
verify_installed "ansible-playbook"

if [ ! -f ${DIR}/cp_ansible_playground_id_rsa.pub ]
then
     echo -e "\033[0;33mINFO: Generating ssh keys.\033[0m"
     # generate ssh keys (with no prompt)
    ssh-keygen -t rsa -f ${DIR}/cp_ansible_playground_id_rsa -q -N ""
fi
...
# copy custom files
cp ${DIR}/ansible.cfg ${DIR}/cp-ansible/
...
echo -e "\033[0;33mINFO: Checking Ansible can connect over SSH.\033[0m"
