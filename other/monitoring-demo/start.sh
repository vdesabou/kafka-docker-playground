...
then
     ...
     docker exec connect bash -c 'kafka-avro-console-consumer --topic mysql-application --bootstrap-server broker:9092 --property schema.registry.url="http://schema-registry:8081" --from-beginning --max-messages 2'
fi
...
if [[ ! $(type kafka-consumer-groups 2>&1) =~ "not found" ]]; then
     ...
     kafka-consumer-groups --bootstrap-server "broker:9092" --group simple-stream --describe
fi
