...
for i in broker ldap ldap2 ldap3
do
    echo "------------------------------- $i -------------------------------"
...
EOF
    ...
    keytool -noprompt -keystore /tmp/kafka.$i.keystore.jks -alias CARoot -import -file /tmp/snakeoil-ca-1.crt -storepass confluent -keypass confluent
     ...
     keytool -noprompt -keystore /tmp/kafka.$i.keystore.jks -alias $i -import -file /tmp/$i-ca1-signed.crt -storepass confluent -keypass confluent -ext "SAN=dns:$i,dns:localhost,dns:$i.confluent.io"
        #keytool -list -v -keystore kafka.$i.keystore.jks -storepass confluent
