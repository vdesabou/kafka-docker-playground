...
clear_traffic_control() {
    ...
    docker exec -u0 -t $src_container tc qdisc del dev eth0 root 2>&1
}
...
get_latency() {
    ...
    src_container=$1
    dst_container=$2
    docker exec -u0 -t $src_container ping $dst_container -c 4  | tail -1 | awk -F '/' '{print $5}'
}
...
add_latency() {
    ...
    docker exec -u0 -t $src_container tc qdisc add dev eth0 root handle 1: prio 2>&1
    ...
    docker exec -u0 -t $src_container tc filter add dev eth0 protocol ip parent 1: prio 1 u32 match ip dst $dst_container_ip flowid 1:1 2>&1
        
    # Route the rest of the of the packets without any control.
    # Add a filter to the parent queue 1:. The filter has priority 2.
    docker exec -u0 -t $src_container tc filter add dev eth0 protocol all parent 1: prio 2 u32 match ip dst 0.0.0.0/0 flowid 1:2 2>&1
    docker exec -u0 -t $src_container tc filter add dev eth0 protocol all parent 1: prio 2 u32 match ip protocol 1 0xff flowid 1:2 2>&1
    
    # Add a child queue named 10: under class 1:1. All outgoing packets that will be routed to 10: will have delay applied them.
    docker exec -u0 -t $src_container tc qdisc add dev eth0 parent 1:1 handle 10: netem delay $latency 2>&1

    # Add a child queue named 20: under class 1:2 
    docker exec -u0 -t $src_container tc qdisc add dev eth0 parent 1:2 handle 20: sfq 2>&1
