...
playground connector create-or-update --connector mysql-ssl-source  << EOF
{
    ...
    "tasks.max":"1",
    "connection.url": "jdbc:mysql://mysql:3306/mydb?user=userssl&password=password&verifyServerCertificate=true&useSSL=true&requireSSL=true&enabledTLSProtocols=TLSv1,TLSv1.1,TLSv1.2,TLSv1.3&trustCertificateKeyStoreUrl=file:///etc/kafka/secrets/truststore.jks&trustCertificateKeyStorePassword=mypassword",
