...
then
     log "Creating JSON CONVERTER HDFS Sink connector without Hive integration (not supported with JDK 11)"
     curl -X PUT \
          -H "Content-Type: application/json" \
          --data '{
     ...
     log "Creating JSON CONVERTER HDFS Sink connector with Hive integration"
     curl -X PUT \
          -H "Content-Type: application/json" \
          --data '{
                    "connector.class":"io.confluent.connect.hdfs3.Hdfs3SinkConnector",
