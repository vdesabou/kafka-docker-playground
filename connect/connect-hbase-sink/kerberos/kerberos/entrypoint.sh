...
create_hackolade_user() {
	...
	kadmin -p $KERB_ADMIN_USER/admin -w $KERB_ADMIN_PASS -q "addprinc -randkey nn/$(hostname -f)@${REALM}"
	kadmin -p $KERB_ADMIN_USER/admin -w $KERB_ADMIN_PASS -q "addprinc -randkey dn/$(hostname -f)@${REALM}"
	kadmin -p $KERB_ADMIN_USER/admin -w $KERB_ADMIN_PASS -q "addprinc -randkey HTTP/$(hostname -f)@${REALM}"
	kadmin -p $KERB_ADMIN_USER/admin -w $KERB_ADMIN_PASS -q "addprinc -randkey jhs/$(hostname -f)@${REALM}"
	kadmin -p $KERB_ADMIN_USER/admin -w $KERB_ADMIN_PASS -q "addprinc -randkey yarn/$(hostname -f)@${REALM}"
	...
	kadmin -p $KERB_ADMIN_USER/admin -w $KERB_ADMIN_PASS -q "xst -k /opt/keytabs/jhs.service.keytab jhs/$(hostname -f)"
