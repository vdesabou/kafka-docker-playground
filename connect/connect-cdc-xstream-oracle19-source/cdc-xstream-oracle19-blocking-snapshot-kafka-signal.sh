...
log "execute blocking snapshot to capture all records within ORCLCDB.C##CFLTUSER.CUSTOMERS table via signal topic"
playground topic produce -t dbz-signals --nb-messages 1 --key "cflt" << 'EOF'
{"id": "captain adhoc 1","type":"execute-snapshot","data": {"data-collections": ["ORCLCDB.C##CFLTUSER.CUSTOMERS"], "type": "blocking"}}
EOF
