...
USER="awsuser"
PORT="5439"
AccessKeyID=${3:-myAccessKey}
SecretAccessKey=${4:-mySecretKey}
...
curl -X PUT \
     ...
     --data '{
               "connector.class": "io.confluent.connect.jdbc.JdbcSourceConnector",
                    "tasks.max": "1",
                    "connection.url": "jdbc:redshift:iam://'"$CLUSTER"':'"$PORT"'/'"$DATABASE"'?AccessKeyID='"$AccessKeyID"'&SecretAccessKey='"$SecretAccessKey"'&ssl=false&DbUser=awsuser",
