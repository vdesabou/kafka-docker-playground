...
then
     ...
     docker run -i --rm -e KAFKA_CLIENT_TAG=$KAFKA_CLIENT_TAG -v "${DIR}/simple-send":/usr/src/mymaven -v "$HOME/.m2":/root/.m2 -v "${DIR}/simple-send/target:/usr/src/mymaven/target" -w /usr/src/mymaven maven:3.6.1-jdk-11 mvn package
fi
...
AZURE_NAME=playground${USER}eventhubs${GITHUB_RUN_NUMBER}${TAG}
