...
-- Create Iceberg table using Snowflake-managed internal storage.
-- No EXTERNAL_VOLUME is specified; Snowflake uses the account-level default
-- internal volume provisioned and managed by Snowflake.
...
-- Enable schema evolution before transferring ownership
ALTER ICEBERG TABLE TEST_TABLE SET ENABLE_SCHEMA_EVOLUTION = TRUE;

-- Transfer ownership to the connector role so it can perform DDL (add columns etc.)
GRANT OWNERSHIP ON TABLE TEST_TABLE TO ROLE $PLAYGROUND_CONNECTOR_ROLE COPY CURRENT GRANTS;
...
log "Storage: Snowflake-managed internal volume"
log "==========================================="
