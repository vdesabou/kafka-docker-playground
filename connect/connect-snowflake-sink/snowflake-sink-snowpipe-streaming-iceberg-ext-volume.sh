...
then
     ...
     logwarn "Iceberg table storage will not be configured."
     logwarn "To configure later, set AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY and re-run the script."
fi

# https://<account_name>.<region_id>.snowflakecomputing.com:443
SNOWFLAKE_URL="https://$SNOWFLAKE_ACCOUNT_NAME.snowflakecomputing.com"

# Get Snowflake AWS account ID dynamically
# Query Snowflake to get the external volume storage location's AWS account
...
# Extract the AWS account ID from the ARN (format: arn:aws:iam::ACCOUNT_ID:user/...)
...
# Create S3 bucket for Iceberg tables if AWS credentials are provided
if [ ! -z "$AWS_ACCESS_KEY_ID" ] && [ ! -z "$AWS_SECRET_ACCESS_KEY" ]
then
     log "Creating S3 bucket for Iceberg storage: $S3_BUCKET_NAME in region $AWS_REGION"
     set +e
     if [ "$AWS_REGION" = "us-east-1" ]; then
          # us-east-1 doesn't support LocationConstraint parameter
     ...
     log "Configuring S3 bucket versioning and encryption"
     set +e
     aws s3api put-bucket-versioning \
          --bucket "$S3_BUCKET_NAME" \
          --versioning-configuration Status=Enabled
     set -e
     log "Versioning configured"
     ...
     # Create cross-account IAM role for Snowflake access
     log "Creating cross-account IAM role for Snowflake to access S3 bucket"
     SNOWFLAKE_ROLE_NAME="snowflake-iceberg-role"

     # Trust policy allowing Snowflake IAM user to assume this role
     # Using the Snowflake IAM user ARN extracted from DESCRIBE INTEGRATION
     TRUST_POLICY='{
          "Version": "2012-10-17",
          "Statement": [
               {
     ...
     # Delete the role if it exists (to ensure clean state with updated trust policy)
     log "Checking for existing role and recreating with updated trust policy..."
     set +e
     ...
     # Create the role with the updated trust policy
     aws iam create-role \
     ...
     # Note: Snowflake needs access to all paths for verification and table management
     S3_POLICY='{
          "Version": "2012-10-17",
          "Statement": [
               {
     ...
     # Delete old policy if it exists, then attach the new one
     set +e
     ...
     SNOWFLAKE_ROLE_ARN="arn:aws:iam::$AWS_ACCOUNT_ID:role/$SNOWFLAKE_ROLE_NAME"
     log "Snowflake IAM Role ARN: $SNOWFLAKE_ROLE_ARN"
fi
...
docker run --quiet --rm -i -e SNOWSQL_PWD="$SNOWFLAKE_PASSWORD" -e RSA_PUBLIC_KEY="$RSA_PUBLIC_KEY" kurron/snowsql --username $SNOWFLAKE_USERNAME -a $SNOWFLAKE_ACCOUNT_NAME << EOF
USE ROLE SECURITYADMIN;
-- Grant Iceberg-specific permissions in the schema
...
-- Grant permissions for managing Iceberg tables
GRANT ALL ON FUTURE ICEBERG TABLES IN SCHEMA $PLAYGROUND_DB.PUBLIC TO ROLE $PLAYGROUND_CONNECTOR_ROLE;
EOF

# Configure S3 integration in Snowflake for Iceberg storage
if [ ! -z "$AWS_ACCESS_KEY_ID" ] && [ ! -z "$AWS_SECRET_ACCESS_KEY" ]
then
     log "Creating Snowflake integration and storage location for S3 Iceberg storage"
...
EOF
     ...
     log "Note: Verify the IAM role ARN (arn:aws:iam::$AWS_ACCOUNT_ID:role/snowflake-role) exists in your AWS account"
fi
...
playground topic produce -t test_table --nb-messages 9 << 'EOF'
{
  "fields": [
    {
      "name": "u_name",
