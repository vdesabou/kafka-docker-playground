...
AZURE_NAME=pg${USER}ck${GITHUB_RUN_NUMBER}${TAG_BASE}
...
playground container exec --container broker --command "kafka-topics --bootstrap-server 127.0.0.1:9092 --create --topic apparels --partitions 1 --replication-factor 1"

log "Send messages to Azure Cosmos DB"
playground container exec --container azure-cosmos-client --command "export AZURE_COSMOSDB_DB_ENDPOINT_URI=\"$AZURE_COSMOSDB_DB_ENDPOINT_URI\"; export AZURE_COSMOSDB_PRIMARY_CONNECTION_KEY=\"$AZURE_COSMOSDB_PRIMARY_CONNECTION_KEY\"; export AZURE_COSMOSDB_DB_NAME=\"$AZURE_COSMOSDB_DB_NAME\"; export AZURE_COSMOSDB_CONTAINER_NAME=\"$AZURE_COSMOSDB_CONTAINER_NAME\"; python /insert-data.py"
