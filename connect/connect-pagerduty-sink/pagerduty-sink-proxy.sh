...
if [ ! -z "$TAG_BASE" ] && version_gt $TAG_BASE "7.9.99" && [ ! -z "$CONNECTOR_TAG" ] && ! version_gt $CONNECTOR_TAG "0.0.99"
then
...
playground connector create-or-update --connector pagerduty-sink  << EOF
{
  ...
  "pagerduty.proxy.url": "http://squid:8888",
  "pagerduty.proxy.user": "admin",
  "pagerduty.proxy.password": "1234",
  "tasks.max": "1",
