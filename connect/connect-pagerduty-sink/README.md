...
```bash
$ just use <playground run> command and search for pagerduty-sink<use tab key to activate fzf completion (see https://kafka-docker-playground.io/#/cli?id=%e2%9a%a1-setup-completion), otherwise use full path, or correct relative path> <PAGERDUTY_USER_EMAIL> <PAGERDUTY_API_KEY> .sh in this folder
```
