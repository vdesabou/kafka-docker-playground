...
if [ ! -z "$ORACLE_DATAGEN" ]
then
     log "🌪️ ORACLE_DATAGEN is set, make sure to increase redo.log.row.fetch.size, have a look at https://github.com/vdesabou/kafka-docker-playground/blob/master/connect/connect-cdc-oracle21-source/README.md#note-on-redologrowfetchsize"
...
for script in ../../connect/connect-cdc-oracle21-source/sample-sql-scripts/*.sh
do
     $script "ORCLPDB1"
done
...
set +e
timeout 60 docker exec connect kafka-avro-console-consumer -bootstrap-server broker:9092 --property schema.registry.url=http://schema-registry:8081 --topic ORCLPDB1.C__MYUSER.CUSTOMERS --from-beginning --max-messages 13 --property print.key=true > /tmp/result.log  2>&1
set -e
cat /tmp/result.log
...
then
     ...
     docker exec -d oracle-datagen bash -c "java ${JAVA_OPTS} -jar oracle-datagen-1.0-SNAPSHOT-jar-with-dependencies.jar --host oracle --username C##MYUSER --password mypassword --sidOrServerName service_name --sidOrServerNameVal ORCLPDB1 --maxPoolSize 10 --durationTimeMin $DURATION"
fi
