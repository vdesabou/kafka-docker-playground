...
log "Creating http-sink connector with the Apache ReplaceField SMT (org.apache.kafka.connect.transforms) renaming field 'email' to 'email_apache'"
playground connector create-or-update --connector http-sink  << EOF
{
     ...
     "transforms.renameEmail.renames": "email:email_apache"
}
EOF
...
log "Verify the Apache ReplaceField SMT was applied: 'email' should now appear as 'email_apache' in the body received by the HTTP server"
playground container logs --container httpserver --wait-for-log "email_apache"
