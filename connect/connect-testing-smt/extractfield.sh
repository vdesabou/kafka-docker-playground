...
{
    "id": "iteration.index",
    "extracted_value": "faker.internet.userName()",
...
log "Creating http-sink connector with the Apache ExtractField SMT (org.apache.kafka.connect.transforms) extracting the 'extracted_value' field as the record value"
playground connector create-or-update --connector http-sink  << EOF
{
     ...
     "transforms.extract.field": "extracted_value"
}
EOF

sleep 10

log "Check the success-responses topic (ExtractField SMT loaded, applied and records flowed to the HTTP server)"
