...
log "Creating http-sink connector with the Apache Filter SMT (org.apache.kafka.connect.transforms) + RecordIsTombstone predicate to drop tombstone records"
...
log "Check the success-responses topic: only the 5 regular records should reach the HTTP server, the 3 tombstones are dropped by the Filter"
