...
log "Creating http-sink connector with the Apache InsertField SMT (org.apache.kafka.connect.transforms) inserting a static field"
playground connector create-or-update --connector http-sink  << EOF
{
     ...
     "transforms.insertField.static.field": "inserted_field",
     "transforms.insertField.static.value": "smt-inserted-value"
}
EOF
...
log "Verify the Apache InsertField SMT was applied: the inserted static value appears in the body received by the HTTP server"
playground container logs --container httpserver --wait-for-log "smt-inserted-value"
