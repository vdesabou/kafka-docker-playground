...
log "Sending 10 messages (with a string 'level' field) to topic http-messages"
...
log "Creating http-sink connector with the Apache Cast SMT (org.apache.kafka.connect.transforms) casting the 'level' field to int32"
playground connector create-or-update --connector http-sink  << EOF
{
     ...
     "transforms": "castLevel",
     "transforms.castLevel.type": "org.apache.kafka.connect.transforms.Cast\$Value",
     "transforms.castLevel.spec": "level:int32"
}
EOF

sleep 10

log "Check the success-responses topic (Cast SMT loaded, applied and records flowed to the HTTP server)"
