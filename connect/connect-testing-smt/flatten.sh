...
log "Creating http-sink connector with the Apache Flatten SMT (org.apache.kafka.connect.transforms) flattening the nested structure"
...
log "Verify the Apache Flatten SMT was applied: the nested field appears flattened as 'nested.inner' in the body received by the HTTP server"
