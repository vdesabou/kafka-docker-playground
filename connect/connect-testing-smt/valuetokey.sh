...
log "Creating http-sink connector with the Apache ValueToKey SMT (org.apache.kafka.connect.transforms) forming the key from the 'id' value field"
...
log "Check the success-responses topic: all 10 records still flow to the HTTP server with the ValueToKey SMT applied"
