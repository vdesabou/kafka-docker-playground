...
log "Creating http-sink connector with the Apache MaskField SMT (org.apache.kafka.connect.transforms) masking the 'email' field"
playground connector create-or-update --connector http-sink  << EOF
{
     ...
     "transforms.maskEmail.replacement": "masked-value"
}
EOF
...
log "Verify the Apache MaskField SMT was applied: the 'email' field is replaced by the mask in the body received by the HTTP server"
playground container logs --container httpserver --wait-for-log "masked-value"
