...
# TimestampRouter rewrites the target topic from the record timestamp. It cannot run on a datagen
# source (datagen builds its SourceRecord with a null timestamp, so new Date(timestamp) would NPE),
# so it is exercised here as an HTTP sink: sink records always carry a Kafka message timestamp. The
# topic is rewritten harmlessly (the HTTP sink posts to a single URL), so the check is that records
# still flow with the SMT applied.
...
log "Sending 10 messages to topic http-messages"
...
log "Creating http-sink connector with the Apache TimestampRouter SMT (org.apache.kafka.connect.transforms) rewriting the topic to <topic>-<yyyyMMdd>"
...
log "Check the success-responses topic: all 10 records still flow with the TimestampRouter SMT applied"
