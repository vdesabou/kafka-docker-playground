...
log "Sending 10 nested messages to topic http-messages"
playground topic produce -t http-messages --nb-messages 10 << 'EOF'
{
    "id": "iteration.index",
    "nested": {
        "inner": "faker.internet.userName()"
    }
}
EOF
...
log "Creating http-sink connector with the Confluent Flatten SMT (io.confluent.connect.transforms) flattening the nested structure"
...
log "Verify the Confluent Flatten SMT was applied: the nested field appears flattened as 'nested.inner' in the body received by the HTTP server"
playground container logs --container httpserver --wait-for-log "nested.inner"
