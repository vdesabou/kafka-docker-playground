...
playground connector create-or-update --connector debezium-mongodb-source  << EOF
{
    ...
    "_signal.data.collection": "inventory.debezium_signal",
...
log "Trigger Ad hoc snapshot via kafka signal topic"
playground topic produce -t dbz-signals --nb-messages 1 --key "dbserver1" << 'EOF'
{"id": "captain adhoc 1","type":"execute-snapshot","data": {"data-collections": ["inventory.customers2"], "type": "incremental"}}
EOF
