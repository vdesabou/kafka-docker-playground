...
if [ ! -z "$TAG_BASE" ] && version_gt $TAG_BASE "7.9.99" && [ ! -z "$CONNECTOR_TAG" ] && ! version_gt $CONNECTOR_TAG "2.9.99"
then
...
if [ ! -f debezium-connector-mariadb-${CONNECTOR_TAG}.tar.gz ]
then
    curl -L -o debezium-connector-mariadb-${CONNECTOR_TAG}.tar.gz https://repo1.maven.org/maven2/io/debezium/debezium-connector-mariadb/${CONNECTOR_TAG}.Final/debezium-connector-mariadb-${CONNECTOR_TAG}.Final-plugin.tar.gz
    tar xvfz debezium-connector-mariadb-${CONNECTOR_TAG}.tar.gz
fi
...
playground connector create-or-update --connector debezium-mariadb-source  << EOF
{
