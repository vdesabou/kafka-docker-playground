...
do
    ...
    cp ${PWD}/${component}/target/awscredentialsprovider-1.0.0-jar-with-dependencies.jar ../../confluent-hub/confluentinc-kafka-connect-aws-dynamodb/lib/awscredentialsprovider-1.0.0-jar-with-dependencies.jar
    set -e
done
...
playground connector create-or-update --connector dynamodb-sink  << EOF
{
    ...
    "aws.dynamodb.credentials.provider.sts.aws.access.key.id": "$AWS_ACCOUNT_WITH_ASSUME_ROLE_AWS_ACCESS_KEY_ID",
    "aws.dynamodb.credentials.provider.sts.aws.secret.key.id": "$AWS_ACCOUNT_WITH_ASSUME_ROLE_AWS_SECRET_ACCESS_KEY",
    "confluent.license": "",
