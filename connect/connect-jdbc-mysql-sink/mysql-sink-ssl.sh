...
playground connector create-or-update --connector mysql-ssl-sink  << EOF
{
  ...
  "tasks.max": "1",
  "connection.url": "jdbc:mysql://mysql:3306/db?user=userssl&password=password&verifyServerCertificate=true&useSSL=true&requireSSL=true&enabledTLSProtocols=TLSv1,TLSv1.1,TLSv1.2,TLSv1.3&trustCertificateKeyStoreUrl=file:///etc/kafka/secrets/truststore.jks&trustCertificateKeyStorePassword=mypassword",
  "topics": "orders",
  "auto.create": "true"
}
EOF
...
playground container exec --container mysql --command "bash -c \"mysql --protocol=TCP --host=127.0.0.1 --user=userssl --password=password --ssl-mode=VERIFY_CA --ssl-ca=/var/lib/mysql/ca.pem --ssl-cert=/var/lib/mysql/client-cert.pem --ssl-key=/var/lib/mysql/client-key.pem --database=db -e 'describe orders'\""

log "Show content of orders table:"
playground container exec --container mysql --command "bash -c \"mysql --protocol=TCP --host=127.0.0.1 --user=userssl --password=password --ssl-mode=VERIFY_CA --ssl-ca=/var/lib/mysql/ca.pem --ssl-cert=/var/lib/mysql/client-cert.pem --ssl-key=/var/lib/mysql/client-key.pem --database=db -e 'select * from orders'\"" > /tmp/result.log  2>&1
cat /tmp/result.log
