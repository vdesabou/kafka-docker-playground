...
```bash
$ aws_docker_cli sqs create-queue --queue-name sqs-source-connector-demo
```
...
```bash
$ aws_docker_cli sqs create-queue --queue-name sqs-source-connector-demo-ssl
```
...
docker exec -e QUEUE_URL="$QUEUE_URL" connect \
     ...
     https://localhost:8083/connectors/sqs-source/config | jq_docker_cli .
```
...
```bash
$ aws_docker_cli sqs create-queue --queue-name sqs-source-connector-demo-sasl-ssl
```
