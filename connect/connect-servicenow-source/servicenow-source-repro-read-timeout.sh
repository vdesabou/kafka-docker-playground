...
# docker exec toxiproxy sh -c "/go/bin/toxiproxy-cli inspect servicenow"
...
curl -X PUT \
     ...
     http://localhost:8083/connectors/servicenow-source-toxiproxy/config | jq .


sleep 10

log "Create one record to ServiceNow"
docker exec -e SERVICENOW_URL="$SERVICENOW_URL" -e SERVICENOW_PASSWORD="$SERVICENOW_PASSWORD" connect bash -c "export HTTP_PROXY=toxiproxy:8888 && export HTTPS_PROXY=toxiproxy:8888 && \
   curl -X POST \
