...
curl -X PUT \
     ...
     https://localhost:8083/connectors/servicenow-source/config | jq .
...
timeout 60 docker exec connect kafka-console-consumer -bootstrap-server broker:9092 --topic topic-servicenow --consumer.config /etc/kafka/secrets/client_without_interceptors.config --from-beginning --max-messages 1