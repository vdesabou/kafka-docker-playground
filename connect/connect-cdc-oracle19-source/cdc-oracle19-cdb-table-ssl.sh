...
log "Verifying topic ORCLCDB.C__MYUSER.CUSTOMERS: there should be 11 records"
timeout 60 docker exec connect kafka-avro-console-consumer -bootstrap-server broker:9092 --property schema.registry.url=http://schema-registry:8081 --topic ORCLCDB.C__MYUSER.CUSTOMERS --from-beginning --max-messages 11 > /tmp/result.log  2>&1
cat /tmp/result.log
