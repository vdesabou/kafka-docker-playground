...
seq -f "{\"f1\": \"value%g\"}" 10 | docker container exec -i connect kafka-avro-console-producer --broker-list kafka1:9091 --topic gcs_topic --property value.schema='{"type":"record","name":"myrecord","fields":[{"name":"f1","type":"string"}]}' --property schema.registry.url=https://schemaregistry:8085 --producer.config /etc/kafka/secrets/client_without_interceptors.config
...
#                     "confluent.topic.ssl.principal.mapping.rules" : "RULE:^CN=(.*?),OU=TEST.*$/$1/,DEFAULT"
#           }}' \
