...
docker container exec broker-europe timeout 120 kafka-console-consumer --bootstrap-server localhost:9092 --include "sales_.*" --from-beginning --max-messages 20 --property metadata.max.age.ms 30000

log "Verify we have received the data in all the sales_ topics in the US"
docker container exec broker-us timeout 120 kafka-console-consumer --bootstrap-server localhost:9092 --include "sales_.*" --from-beginning --max-messages 20 --property metadata.max.age.ms 30000
